...
ALTER TABLE BILL ALTER COLUMN bill_id SET DEFAULT nextval('bill_id_seq');
ALTER TABLE BILL ALTER COLUMN bill_id SET NOT NULL;
ALTER SEQUENCE bill_id_seq OWNED BY bill.bill_id;
SELECT setval('bill_id_seq', 500010, true);

CREATE SEQUENCE patient_id_seq;
ALTER TABLE PATIENT ALTER COLUMN pid SET DEFAULT nextval('patient_id_seq');
ALTER TABLE PATIENT ALTER COLUMN pid SET NOT NULL;
ALTER SEQUENCE patient_id_seq OWNED BY patient.pid;
SELECT setval('patient_id_seq', 3010, true);

CREATE SEQUENCE employee_id_seq;
ALTER TABLE EMPLOYEE ALTER COLUMN eid SET DEFAULT nextval('employee_id_seq');
ALTER TABLE EMPLOYEE ALTER COLUMN eid SET NOT NULL;
ALTER SEQUENCE employee_id_seq OWNED BY employee.eid;
SELECT setval('employee_id_seq', 1008, true);

CREATE ROLE employees1;
CREATE ROLE employees2;
CREATE ROLE employees3;
CREATE USER pes1001 WITH LOGIN PASSWORD 'rameshr';
CREATE USER pes1002 WITH LOGIN PASSWORD 'angelinas';
CREATE USER pes1003 WITH LOGIN PASSWORD 'sureshm';
CREATE USER pes1004 WITH LOGIN PASSWORD 'rupeshd';
CREATE USER pes1005 WITH LOGIN PASSWORD 'tushars';
CREATE USER pes1006 WITH LOGIN PASSWORD 'vishnug';
CREATE USER pes1007 WITH LOGIN PASSWORD 'ravikumars';
CREATE USER pes1008 WITH LOGIN PASSWORD 'sumam';
GRANT employees1 to pes1001,pes1003,pes1006;
GRANT employees2 to pes1002,pes1007;
GRANT employees3 to pes1004,pes1005,pes1008;

ALTER TABLE Associated_With enable row level security;
ALTER TABLE contains enable row level security;
ALTER TABLE EMPLOYEE enable row level security;
ALTER TABLE PATIENT enable row level security;
ALTER TABLE BILL enable row level security;

CREATE POLICY emp_restric1 on Associated_With to employees1 using (store_id=1);
CREATE POLICY emp_restric1 on contains to employees1 using (store_id=1);
CREATE POLICY emp_restric1 on EMPLOYEE to employees1 using (store_id=1);
CREATE POLICY emp_restric1 on PATIENT to employees1 using (store_id=1);
CREATE POLICY emp_restric1 on BILL to employees1 using (store_id=1);

CREATE POLICY emp_restric2 on Associated_With to employees2 using (store_id=2);
CREATE POLICY emp_restric2 on contains to employees2 using (store_id=2);
CREATE POLICY emp_restric2 on EMPLOYEE to employees2 using (store_id=2);
CREATE POLICY emp_restric2 on PATIENT to employees2 using (store_id=2);
CREATE POLICY emp_restric2 on BILL to employees2 using (store_id=2);

CREATE POLICY emp_restric3 on Associated_With to employees3 using (store_id=3);
CREATE POLICY emp_restric3 on contains to employees3 using (store_id=3);
CREATE POLICY emp_restric3 on EMPLOYEE to employees3 using (store_id=3);
CREATE POLICY emp_restric3 on PATIENT to employees3 using (store_id=3);
CREATE POLICY emp_restric3 on BILL to employees3 using (store_id=3);

GRANT SELECT,UPDATE,DELETE,INSERT on contains,PATIENT,BILL to employees1,employees2,employees3;
GRANT SELECT on Associated_With,EMPLOYEE,PHARMACY,DOCTOR,MEDICINE to employees1,employees2,employees3;
GRANT USAGE,SELECT on SEQUENCE employee_id_seq,patient_id_seq,bill_id_seq to employees1,employees2,employees3;

CREATE FUNCTION decrease() RETURNS trigger AS $$
BEGIN
update contains set quantity=quantity-new.quantity where drug_id=new.drug_id and store_id=new.store_id;
RETURN NULL;
END;
$$ LANGUAGE plpgsql;
